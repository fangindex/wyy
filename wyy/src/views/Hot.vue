<template>
  <div class="Hot" :class="{ RecommendActive: currentMusic }">
    <HotSongList :updateTime="updateTime"></HotSongList>

    <div class="loading" v-if="$root.isShowLoadings">
      <img
        src="data:image/gif;base64,"
        alt=""
      />
    </div>
    <SongItem2
    :currentMusic="currentMusic"
      v-else
      @translate-music="$emit('translate-music', $event)"
      @translate-index="$emit('translate-index', newSong)"
      v-for="(item, index) in newSong"
      :item="item"
      :index="index"
      :key="index"
      :pause="pause"
    ></SongItem2>
  </div>
</template>

<script>
import SongItem2 from "@/components/SongItem2.vue";
import HotSongList from "@/components/HotSongList.vue";
export default {
  props:["currentMusic","pause"],
  components: {
    SongItem2,
    HotSongList,
  },
  data: function () {
    return {
      songName: [],
      srtSongName: "",
      songNames: [],
      newSong: [],
      count: 0,
      updateTime:null,
    };
  },
  created() {
    this.$root.isShowLoadings = true;
    this.axios.get("/top/list?idx=1").then((response) => {
      this.updateTime=response.data.playlist.updateTime;
      this.songName = response.data.playlist.trackIds.slice(0, 20);
      for (var i = 0; i < this.songName.length; i++) {
        if (i < this.songName.length - 1) {
          this.srtSongName += this.songName[i].id + ",";
        } else {
          this.srtSongName += this.songName[i].id;
        }
      }
      this.axios
        .get("/song/detail?ids=" + this.srtSongName)
        .then((response) => {
          this.songNames = response.data.songs;
          for (i in this.songNames) {
            var k = {};
            k.name = this.songNames[i].name;
            k.picUrl = this.songNames[i].al.picUrl;
            k.song = { artists: this.songNames[i].ar };
            k.id = this.songNames[i].id;
            this.newSong.push(k);
          }
        });
      this.count += 1;
    });
  },
  watch: {
    count: function () {
      if (this.count) {
        this.$root.isShowLoadings = false;
      }
    },
  },
};
</script>

<style lang="less">
.Hot {
  margin-top: 40px;
  .loading{
    text-align: center;
    margin-top: 25px;
  }
}
</style>